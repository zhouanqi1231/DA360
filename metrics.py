...
def compute_scale_and_shift(prediction, target, mask):
        M_x2 = torch.mean(prediction[mask]**2)
        M_x = torch.mean(prediction[mask])
        Sxx = M_x2-M_x**2

        M_y = torch.mean(target[mask])
        M_xy = torch.mean(prediction[mask] * target[mask])
...
def compute_depth_metrics(gt, pred, mask=None, alignment=0, max_depth=1000.0):
    ...
    if alignment==1:
        scale = torch.mean(gt[mask&(gt<=max_depth)])/(torch.mean(pred[mask&(gt<=max_depth)]))
        shift = 0
    elif alignment==2:
        scale, shift = compute_scale_and_shift(pred, gt, mask&(gt<=max_depth))
    else:
        scale, shift = 1, 0
...
class Evaluator(object):
    ...
    def __init__(self, alignment=0, max_depth=128.0):

        self.alignment = alignment
        self.max_depth = max_depth
        # Error and Accuracy metric trackers
        self.metrics = {}
        self.metrics["err/abs_err"] = AverageMeter()
    ...
    def reset_eval_metrics(self):
        ...
        """
        self.metrics["err/abs_err"].reset()
    ...
    def compute_eval_metrics(self, gt_depth, pred_depth, mask):
        ...
        abs_, abs_rel, sq_rel, rms, rms_log, log10, a1, a2, a3 = \
            compute_depth_metrics(gt_depth, pred_depth, mask, self.alignment, self.max_depth)
    ...
    def print(self, dir=None):
        ...
        print("\n  "+ ("{:>9} | " * 9).format("abs_err", "abs_rel", "sq_rel", "rms", "rms_log", "log10", "a1", "a2", "a3"))
